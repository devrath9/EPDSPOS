...
const AdhrDistrict = () => {
    ...
    const {year, month, district} = useParams()
...
const[TrDistrict, setTrDistrict] = useState([])
...
const fetchDistrictData = async()=>{
    ...
    try{
        ...
        const {data} = await axios.get(backendUrl + `/api/admin/disaadharTrans/${district}/${year}/${month}`)
        if(data.success){
            setTrDistrict(data.DistrictAadharTrans)
            console.log(data.DistrictAadharTrans)
        }
        else{
    ...
    <div className='p-1 border-[1.5px] border-gray-300 rounded'>
      ...
      <div className=' overflow-x-auto mt-3 mx-1 border-[1.5px] p-1 border-gray-300 rounded'>
               <table ref={tableRef} className="min-w-full table-fixed xl:table-auto border-collapse border border-gray-200 text-sm ">
                   ...
                   <tbody>
                   {TrDistrict.slice(0,TrDistrict.length-1).map((item, index)=>(<tr key={index} className='border-b-2 border-gray-300 font-semibold text-gray-700'>
                           <td className="text-center px-4 py-2"> {index+1}</td>
                           <td onClick={()=>navigate(`/transactionSummary/ByAadhar/${year}/${month}/${district}/${item.Block_ID}`)}
                   ...
                   { <tfoot>
                        {TrDistrict.slice(-1).map((item, index)=>(<tr key={index} className='border-b-2 border-gray-300 font-bold text-white bg-[#2191c0]'>
              ...
              <div className='ml-2 mt-1 text-sm font-semibold text-white'>
                            ...
                            <DownloadTableExcel
                                    filename="Trans_Summary_byAadhar District Report"
